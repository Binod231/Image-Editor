...
def generate_filename(original_name, operation):
    ...
    return f"{base}_{operation}_{timestamp}{ext_map.get(operation, '.png')}{ext}"
...
def home():
    # Check if the user is logged in (has the access token cookie)
    if not request.cookies.get('access_token_cookie'):
        return redirect(url_for('login'))  # Redirect to login if not logged in
    return render_template("index.html")  # Show index page after successful login
...
def login():
    # If already logged in (has access token cookie), redirect to home
    ...
    if request.method == "POST":
        ...
        if email in users and users[email]['password'] == password:
            ...
            response.set_cookie("access_token_cookie", access_token, httponly=True)  # Added httponly for security
